...
const TimeTempTableBody: React.FC = () => {
  ...
  const TempRows =
    ...
    timeTemp.map((thisTimeTemp, i) => {
      const { time, temp, addedCoals } = thisTimeTemp;

      const formattedTime = getFormattedTime(time);

      const tempDiff = i ? temp - timeTemp[i - 1].temp : 0;
      ...
      return <TimeTempTableRow key={time} {...props} />;
    });

  return <TableBody>{TempRows}</TableBody>;
};
